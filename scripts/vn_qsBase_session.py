...
#5. Avoid joins that can multiply rows."""
    ...
    def generate_and_correct_sql(self, question: str, **kwargs) -> str:
        sql = self.generate_sql(question, **kwargs)[0]
    ...
    def get_correction_prompt(self, question, sql, message, **kwargs) -> str:
        initial_prompt = f"You are a {self.dialect} expert. There is a SQL query generated based on the following Database Schema to respond to the Question. Executing this SQL has resulted in an error and you need to fix it based on the error message, while following the system-interpretation of the question. \n"
