...
greetings_English = """
Welcome!\n
I'm your **Assistance‑System**, designed for **multi-turn interactions**. While exploring the data, you can ask follow-up questions & refine the results step by step.\n
In addition to text-to-SQL-translation, I provide **result tables**, **data summaries**, and **auto‑generated diagrams** based on your question.\n
Feel free to **test out the functions** on the **left sidebar** — you can **expand or collapse** it as needed. \n
You can also **adjust the output** and switch the **response language** at any time.
"""

greetings_German = """
Willkommen!\n 
Ich bin Ihr **Assistance‑System**, das für **mehrturnige Interaktionen** konzipiert ist. Während der Datenexploration können Sie Folgefragen stellen & die Ergebnisse schrittweise verfeinern.\n
Neben der Text-zu-SQL-Übersetzung stelle ich **Ergebnistabellen**, **Datenzusammenfassungen** sowie **automatisch generierte Diagramme** auf Basis Ihrer Fragen bereit. \n
Nutzen Sie gerne die **Funktionen in der linken Seitenleiste** — Sie können sie bei Bedarf **ein- oder ausklappen**.\n
Darüber hinaus können Sie die **Ausgabe anpassen** und die **Antwortsprache** jederzeit wechseln.
"""

def greet_user():
    language = vc.get_runtimeParams()["language"]
    greetings = eval("greetings_" + language) 
...
def generate_response(prompt, prePrint=False):
    with st.chat_message("assistant"):
        ...
        if st.session_state.get("show_interpretation") or st.session_state.get("show_alternatives"):
        ...
        sql = vc.generate_sql_cached(prompt, db_mono) #,plan)
        if sql:
            ...
            if st.session_state.get("df") is not None:
                ...
                if st.session_state.get("show_summary"):
                    if not st.session_state.get("show_alternatives"):
                        alternatives = None
...
with st.sidebar:
    ...
    current_lang = vc.get_runtimeParams()['language']
    ...
    with st.expander('Table Preview'):
        with st.form('table',clear_on_submit=True):
            selectbox_val = st.selectbox(
                "Explore your tables",
            ...
            if submitted:
                if(selectbox_val):
                    ...
                    st.warning("Please choose a table first.")
                time.sleep(1)
                app_reload()
    ...
    with st.expander('Output Settings'):
        ...
        st.toggle("Show Alternatives", value=True, key="show_alternatives", disabled = not st.session_state.get("show_summary"))
        with st.form('language',clear_on_submit=True):
            selectbox_lan = st.selectbox(
                "Choose the response language",
                ("English", "German"),
                index=None,
                placeholder= current_lang,
                )
            submitted_lan = st.form_submit_button("Submit")
            if submitted_lan:
                if(selectbox_lan):
                    st.write("You selected:", '*' + selectbox_lan + '*')
                    message = vc.setUp_newLanguage(selectbox_lan)
                    st.write(message)
                    if len(st.session_state.messages) < 2:
                        new_chat()
                else:
                    st.write("Please choose a language first.")
                time.sleep(1)
                app_reload()
